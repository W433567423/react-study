...
;(function (global, factory) {
	...
	function popHydrationState(fiber) {
		...
		if (
			fiber.tag !== HostRoot &&
			(fiber.tag !== HostComponent || (shouldDeleteUnhydratedTailInstances(fiber.type) && !shouldSetTextContent(fiber.type, fiber.memoizedProps)))
		) {
			var nextInstance = nextHydratableInstance
	...
	function updateClassInstance(current, workInProgress, ctor, newProps, renderLanes) {
		...
		var oldProps =
			workInProgress.type === workInProgress.elementType ? unresolvedOldProps : resolveDefaultProps(workInProgress.type, unresolvedOldProps)
		instance.props = oldProps
		...
		if (current === null) {
			...
			if (showFallback) {
				...
				_primaryChildFragment2.memoizedState =
					prevOffscreenState === null ? mountSuspenseOffscreenState(renderLanes) : updateSuspenseOffscreenState(prevOffscreenState, renderLanes)
					...
					if (nextDidTimeout) {
						...
						if ((workInProgress.mode & ConcurrentMode) !== NoMode) {
							...
							var hasInvisibleChildContext =
								current === null && (workInProgress.memoizedProps.unstable_avoidThisFallback !== true || !enableSuspenseAvoidThisFallback)
		...
		if (container._reactRootContainer) {
		...
		} else {
			{
				...
				var isContainerReactRoot =
					container.nodeType === ELEMENT_NODE && isValidContainerLegacy(container.parentNode) && !!container.parentNode._reactRootContainer
